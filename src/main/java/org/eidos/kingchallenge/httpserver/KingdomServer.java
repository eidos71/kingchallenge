	...
	private static final int DELAY_FOR_TERMINATION=0;
	 ...
	 private  ExecutorService serverExecutor;
	 	...
	 	protected void initServer() throws IOException {
	 		LOG.info("Sarting server on {} :  Shutdown port on:{} " ,serverPort,shutdownPort);
	        server = HttpServer.create(new InetSocketAddress(serverPort), HTTP_POOL_CONNECTIONS);
	      //  server.createContext("/herald", new HeraldHandler());
	       // server.createContext("/herald", new GenericPageHandler(HeraldHandler.class));
	        createContext();
	  ...
	  */
	  protected void requestShutdown() {
	    ...
	    protected  <T extends HttpHandler> void  createContext(String context, Class<T> clazz) {
        ...
        for (KingdomHandlerConf confHandler : KingdomHandlerConfManager.getInstance().getHandlerConfList()) {
	            createContext("/" + confHandler.getContext(), confHandler.gethandlerType());
	        }
	    }
	    /**
	     * 
